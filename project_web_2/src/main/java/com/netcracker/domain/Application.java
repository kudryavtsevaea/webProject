...
public class Application {
    ...
    public CommandLineRunner commandLineRunner(ApplicationContext ctx) {
        return args -> {
            ...
            jdbcTemplate.execute("insert into bks (id, author, name, year, pages, info) values (18093322, 'Д.Керуак', 'Бродяги Дхармы', 1958, 173, 'Роман одного из важнейших представителей литературы бит-поколения, Джека Керуака. Произведение было издано в 1958 году издательством Viking Press');" +
            ...
            jdbcTemplate.execute("insert into usr (id, username, password, role) values (1, 'Билоус Антон', '45R6.9EpfU7rS', 'ADMIN');"
                    + "insert into usr (id, username, password, role) values (2, 'Королева Татьяна', '*T9Ny#Q7m[&45xh&', 'USER');"
                    + "insert into usr (id, username, password, role) values (3, 'Григорьев Александр', 't9&8Dpku~Xsx[r+a', 'USER');"
                    + "insert into usr (id, username, password, role) values (4, 'Косыч Илья', '!<b2,NUC$jU;k(@`', 'USER');"
                    + "insert into usr (id, username, password, role) values (5, 'Барсукова Варвара', '=zd7s*q2LT=NY', 'USER');"
                    + "insert into usr (id, username, password, role) values (6, 'Мкоян Карен', 'T/,&xk5CcXBDN4&8', 'USER');"
            );

            List<User> users = jdbcTemplate.query("select id, username, password, role from usr",
